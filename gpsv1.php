...
for ($i=0;$i<$nombre;$i++)
{
    $info[$i] =($array['data'][0]['event']['bodyRaw']);
}
for ($i=0;$i<$nombre;$i++){
...
$reponse = $bdd->query('SELECT DEVICE,NAME,BOAT_NUMBER,BOAT_TYPE FROM fishers');
while ($donnees = $reponse->fetch())
    {
        $fisher[] = array( "device"=>$donnees['DEVICE'],"name"=>$donnees['NAME'], "boat_number"=>$donnees['BOAT_NUMBER'],"boat_type"=>$donnees['BOAT_TYPE']);               
    }
    ...
    
    {
                for($j = 0; $j < sizeof($data);$j++){
                 if ($fisher[$i]['device']==$data[$j]['dev_id']){
                        $json[$k] = array( "name"=>$fisher[$i]['name'],"lat"=>$gps[$j]['lat'], "lon"=>$gps[$j]['lon'],"time"=>$data[$j]['time'],"frequency"=>$data[$j]['frequency'],
                        "rssi"=>$data[$j]  ['rssi']);
                        $k++;
                    }  
            } 
...
$j=1;
    ...
    if ($json[$i]['name']!=$json[$i+1]['name']){
            $actual[$j]=array( "name"=>$jsonr[$i+1]['name'],"lat"=>$json[$i+1]['lat'], "lon"=>$json[$i+1]['lon'],"time "=>$json[$i+1]['time'],"frequency"=>$json[$i+1]['frequency'],
                        "rssi"=>$json[$i+1] ['rssi']);
            $j++;
      }
    }
